...
const addSaleCompany = async (req, res, next) => {
    try {
        const {
            name,
            email,
        ...
        const insertQuery = `INSERT INTO sale_companies (name, email, contact, mobile, address, primary_contact_person, primary_contact_mobile, primary_contact_email, designation, department, company_website, gst_treatment_type, business_legal_name, business_trade_name, pan_number, gst_number, place_of_supply, billing_address, shipping_address, created_by, company_state, company_city, company_pincode) VALUES ('${name}', '${email}', '${contact}', '${mobile}', '${address}', '${primary_contact_person}', '${primary_contact_mobile}', '${primary_contact_email}', '${designation}', '${department}', '${company_website}', '${gst_treatment_type}', '${business_legal_name}', '${business_trade_name}', '${pan_number}', '${gst_number}', '${place_of_supply}', '${billing_address}', '${shipping_address}', '${createdBy}', '${company_state}', '${company_city}', '${company_pincode}')`;
...
const getSaleCompanies = async (req, res, next) => {
    try {
        ...
        if (queryResult.length > process.env.VALUE_ZERO) {
            ...
            if (!pageSize && !isDropdown) {
                ...
                if (type == "1") {
                    filePath = await exportToExcel(values, "client-companies", columns);
                } else {
                    filePath = await exportToPDF(values, "client-companies", "Client Companies", columns);
                }
...
const getSaleCompanyById = async (req, res, next) => {
    try {
...
const editSalesCompany = async (req, res, next) => {
    try {
        const id = req.params.id;
        const { error } = checkPositiveInteger.validate({ id });
...
const updateSalesCompany = async (req, res, next) => {
    try {
        let {
            company_name,
            company_type,
...
const removeSalesCompanyById = async (req, res, next) => {
    try {
        const id = req.params.id;
        const { error } = checkPositiveInteger.validate({ id });
        if (error) return res.status(StatusCodes.FORBIDDEN).json({ status: false, message: error.message });
