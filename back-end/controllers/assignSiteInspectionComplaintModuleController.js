...
const assignSiteInspectionComplaintModule = async (req, res, next) => {
    try {
...
const getSiteInspectionAssignComplaintModuleOnUserId = async (req, res, next) => {
    try {
...
const assignMultipleSiteInspectionComplaintModule = async (req, res, next) => {
    try {
...
async function isComplaintDataExist(data, complaintData) {
    ...
    for (const complaint of complaintData) {
        const matchedItem = data.find(
            (item) => item.module_id.toString() === complaint.module_id && item.module_type === complaint.module_type
        );
...
async function isOutletComplaintsExist(outletData) {
    ...
    for (const complaint of getAllComplaintOnOutlet) {
        const matchedAssignment = getAllAssignComplaint.find((assignment) => assignment.module_id === complaint.id);
...
async function isSaleAreaOutletsExist(outletData) {
    ...
    for (const outlet of complaintSaleAreaOutlets) {
        const matchedAssignment = getAllAssignComplaint.find((assignment) => assignment.module_id === outlet.id);
