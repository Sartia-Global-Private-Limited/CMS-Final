import React, { useEffect, useState } from "react";
import { Card, Col, Nav, Tab } from "react-bootstrap";
import { Helmet } from "react-helmet";
import {
  Link,
  useLocation,
  useNavigate,
  useSearchParams,
} from "react-router-dom";
import { useSelector } from "react-redux";
import { selectUser } from "../../features/auth/authSlice";
import { findMatchingPath } from "../../utils/helper";
import { HEADER_TITLE_DEFAULT } from "../../data/StaticData";
import BankManagement from "./BankManagement/BankManagement";
import AccountManagement from "./AccountManagement/AccountManagement";
import FinancialYear from "../Billings/FinancialYear";
import OrderVia from "../Complaints/OrderVia/OrderVia";
import TaxManagement from "./TaxManagement/TaxManagement";
import BillNoFormat from "./BillNoFormat/BillNoFormat";
import PaymentMethod from "./PaymentMethod/PaymentMethod";
import AddWalletBalance from "./AddWalletBalance";
import EmployeeNoFormat from "./EmployeeNoFormat/EmployeeNoFormat";
import ClientNoFormat from "./ClientAndVendorNoFormat/ClientNoFormat";

const TABS = [
  { component: BankManagement, eventKey: "0" },
  { component: AccountManagement, eventKey: "1" },
  { component: FinancialYear, eventKey: "2" },
  { component: OrderVia, eventKey: "3" },
  { component: TaxManagement, eventKey: "4" },
  { component: BillNoFormat, eventKey: "5" },
  { component: EmployeeNoFormat, eventKey: "6" },
  { component: ClientNoFormat, eventKey: "7" },
  { component: PaymentMethod, eventKey: "8" },
  { component: AddWalletBalance, eventKey: "9" },
];

const MasterDataManagement = () => {
  const { pathname } = useLocation();
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const { userPermission } = useSelector(selectUser);
  const [matchingPathObject, setMatchingPathObject] = useState(null);
  const activeTab = searchParams.get("tab") || "0";

  useEffect(() => {
    if (userPermission) {
      const result = findMatchingPath(userPermission, pathname);
      setMatchingPathObject(result);
      if (!searchParams.get("tab")) {
        navigate(`?tab=0`, { replace: true });
      }
    }
  }, [userPermission, pathname, searchParams, navigate]);

  const checkPermission = matchingPathObject?.modulesOfSubModule[activeTab];

  return (
    <>
      <Helmet>
        <title>MasterData Management · {HEADER_TITLE_DEFAULT}</title>
      </Helmet>
      <Col md={12} data-aos={"fade-up"}>
        <Card className="card-bg p-2">
          <Tab.Container id="tabs" activeKey={activeTab}>
            <Nav
              className="nav-pills flex-nowrap overflow-auto"
              style={{ whiteSpace: "nowrap" }}
              variant="pills"
            >
              {matchingPathObject?.modulesOfSubModule?.map((module, index) => (
                <Nav.Item key={index}>
                  <Nav.Link
                    as={Link}
                    to={`?tab=${index}`}
                    eventKey={`${index}`}
                  >
                    {module.title}
                  </Nav.Link>
                </Nav.Item>
              ))}
            </Nav>

            <Tab.Content>
              {TABS?.map(({ component: Component, eventKey }) => (
                <Tab.Pane className="mt-3" key={eventKey} eventKey={eventKey}>
                  {eventKey == activeTab ? (
                    <Component checkPermission={checkPermission} />
                  ) : (
                    ""
                  )}
                </Tab.Pane>
              ))}
            </Tab.Content>
          </Tab.Container>
        </Card>
      </Col>
    </>
  );
};

export default MasterDataManagement;
