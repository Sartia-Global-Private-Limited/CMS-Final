...
import {
  ...
  getSingleEmployeeNoFormatById,
  postBillNoFormat,
  postEmployeeNoFormat,
  updateBillNoFormat,
  updateEmployeeNoFormat,
} from "../../../services/contractorApi";
import { toast } from "react-toastify";
import { addBillNoFormatSchema, addEmployeeNoFormatSchema } from "../../../utils/formSchema";
...
const CreateEmployeeNoFormat = () => {
  ...
  const fetchEmployeeNoFormatDetails = async () => {
    const res = await getSingleEmployeeNoFormatById(id);
    if (res.status) {
      setEdit(res.data);
    } else {
      setEdit([]);
  ...
  const handleSubmit = async (values, { setSubmitting, resetForm }) => {
    ...
    const res = edit?.id
      ? await updateEmployeeNoFormat(sData)
      : await postEmployeeNoFormat(sData);
    if (res.status) {
  ...
  return (
    <Col md={12} data-aos={"fade-up"}>
      <Helmet>
        <title>
          {type === "view" ? "View" : edit?.id ? "Update" : "Create"} Employee
      ...
      >
        <Formik
          ...
          validationSchema={addEmployeeNoFormatSchema}
                      ...
                      <Form.Group as={Col} md={12}>
                        <div className="shadow p-3">
                          <Row className="g-3 align-items-center">
                            ...
                            <Form.Group as={Col} md={4}>
                              <Form.Label>
                                {t("Start Employee Number")}
                                <span className="text-danger">*</span>
                              </Form.Label>
                              <Form.Control
                                type="text"
                                name="start_employee_number"
                                value={props.values.start_employee_number}
                                onChange={(e) => {
                                  const cleanedValue = e.target.value.replace(
                                    /[^0-9]/g,
                                    ""
                                  );
                                  props.handleChange("start_employee_number")(
                                    cleanedValue
                                  );
                                }}
                                onBlur={props.handleBlur}
                            ...
                            props.values.start_employee_number ? (
                              <Form.Group as={Col} md="4">
                                <Form.Label>
                                  {t("Sample Employee no format")}
                                </Form.Label>
                                <Form.Control
                                  disabled
                                  value={
...
export default CreateEmployeeNoFormat;
