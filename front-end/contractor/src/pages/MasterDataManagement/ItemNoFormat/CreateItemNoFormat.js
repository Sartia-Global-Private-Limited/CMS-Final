...
import { addBillNoFormatSchema, addItemNoFormatSchema } from "../../../utils/formSchema";
...
const CreateItemNoFormat = () => {
  ...
  const fetchItemNoFormatDetails = async () => {
    const res = await getSingleItemNoFormatById(id);
    if (res.status) {
      setEdit(res.data);
    } else {
      setEdit([]);
  ...
  const handleSubmit = async (values, { setSubmitting, resetForm }) => {
    ...
    const res = edit?.id
      ? await updateItemNoFormat(sData)
      : await postItemNoFormat(sData);
    if (res.status) {
  ...
  return (
    <Col md={12} data-aos={"fade-up"}>
      <Helmet>
        <title>
          {type === "view" ? "View" : edit?.id ? "Update" : "Create"} Item
      ...
      >
        <Formik
          ...
          validationSchema={addItemNoFormatSchema}
                        ...
                        <div className="shadow p-3">
                          <Row className="g-3 align-items-center">
                            ...
                            <Form.Group as={Col} md={4}>
                              ...
                              <Form.Control
                                type="text"
                                name="start_item_number"
                                value={props.values.start_item_number}
                                onChange={(e) => {
                                  const cleanedValue = e.target.value.replace(
                                    /[^0-9]/g,
                                    ""
                                  );
                                  props.handleChange("start_item_number")(
                                    cleanedValue
                                  );
                                }}
                                onBlur={props.handleBlur}
                            ...
                            props.values.start_item_number ? (
                              <Form.Group as={Col} md="4">
                                <Form.Label>
                                  {t("Sample Item no format")}
                                </Form.Label>
                                <Form.Control
                                  disabled
                                  value={
...
export default CreateItemNoFormat;
