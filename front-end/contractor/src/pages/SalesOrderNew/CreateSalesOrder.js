...
const CreateSalesOrder = () => {
  ...
  const fetchSaleOrderData = async () => {
    ...
    if (res.status) {
      setEdit(res.data);
      showROApi(res.data?.toCompanyDetails?.unique_id);
    } else {
      setEdit([]);
    }
    setIsLoading(false);
  ...
  const handleCheckPoIsExists = async (e, setFieldValue) => {
    ...
    const res = await getCheckPoIsExists(soNumber);
    if (res.status) {
      toast.error(res.message);
    }
    if (setFieldValue) {
  ...
  return (
    <>
      <Helmet>
        <title>
          {edit?.id ? "Update" : "Create"} Sales Order · CMS Electricals
        </title>
      </Helmet>
                          ...
                          <div className="mt-4 text-center">
                            ...
                            >
                              {props?.isSubmitting ? (
                                ...
                                <>{edit?.id ? "UPDATE" : "CREATE"} Sale Order</>
                              )}
                            </button>
                            <ConfirmAlert
                              size={"sm"}
