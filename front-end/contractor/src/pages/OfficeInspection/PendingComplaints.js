...
import {
  ...
  getOfficeInspectionItemsStock,
  postApprovedUsedItems,
  postAssignComplaint,
  postOfficeInspectionItemsStock,
...
const PendingComplaints = () => {
  ...
  const status = type === "Pending" ? 0 : 1;

  const fetchPendingComplaintsData = async () => {
    const res = await getOfficeInspectionItemsStock(
      search,
      pageSize,
      pageNo,
      id,
  ...
  const handleApproved = async () => {
    const res = await postOfficeInspectionItemsStock(storeId.id);
    if (res.status) {
  ...
  return (
    <>
      <Helmet>
        <title>
          {`All Pending Complaints - ${outletDetails?.complaint_unique_id} · CMS Electricals`}
        </title>
      </Helmet>
      <Col md={12} data-aos={"fade-up"}>
        <CardComponent
          title={`All Pending Complaints - ${outletDetails?.complaint_unique_id}`}
          search={true}
          searchOnChange={(e) => {
          ...
          <div className="table-scroll p-2">
            <Breadcrumb>
              ...
              <Breadcrumb.Item
                linkAs={Link}
                linkProps={{
                  to: `/stock-management/complaints-on-outlet/${prevId}?&&type=${type}`,
                  className: "text-secondary text-decoration-none",
                }}
              >
                    ...
                    {complaintData?.map((e, idx) => (
                      <tr>
                        ...
                        <td>
                          <span className="d-flex align-items-center gap-2">
                            <ImageViewer
                              src={
                                e.user_image
                                  ? `${process.env.REACT_APP_API_URL}${e.user_image}`
                                  : `${process.env.REACT_APP_API_URL}/assets/images/default-image.png`
                              }
                            >
                              <img
                        ...
                        <td>
                          {type === "Pending" ? (
                            <ActionButton
                              eyelink={`/stock-management/view-pending-complaints/${e.id}/${status}`}
                              hideEdit={"d-none"}
                              hideDelete={"d-none"}
                              approveOnclick={() => handleUpdate(e)}
                            />
                          ) : (
                            <ActionButton
                              eyelink={`/stock-management/view-pending-complaints/${e.id}/${status}`}
