...
const MyProfile = () => {
  ...
  function Profile() {
    ...
    const handleUpdatePlan = async () => {
      const res = await getAdminRenewPlanPricing({ plan_id: user?.plan_id });
      if (res.status) {
        toast.success(res.message);
        getProfileData();
      } else {
    ...
    return (
      <Row className="g-4">
                      ...
                      <small>
                        ...
                        <span className="fw-bold">
                          {user?.plan_expire_date
                            ? moment(user?.plan_expire_date).format(
                                "DD-MM-YYYY"
                              )
                            : "-"}
                        </span>
        ...
        <ConfirmAlert
          size={"sm"}
          deleteFunction={handleUpdatePlan}
          hide={setShowAlert}
          show={showAlert}
          defaultIcon={<BsWrenchAdjustableCircle />}
          title={`Confirm Renew`}
          description={`Are you sure you want to renew this!!`}
        />
      </Row>
    );
  }
