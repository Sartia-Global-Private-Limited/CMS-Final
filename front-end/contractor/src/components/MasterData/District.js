...
const District = () => {
  ...
  return (
    <Row className='g-4'>
      <Col md={12}>
        <CardComponent title={'All - District'} icon={<BsPlus />} onclick={() => setDistrictShow(true)} tag={'Add District'}>
            ...
            {[1, 2, 3, 4, 5].map((home) => (
              <div key={home} className='d-align pb-3 justify-content-between hr-border2'>
                ...
                <div className='d-align gap-2'>
                  <BsPencilSquare onClick={() => setDistrictShow(true)} className='social-btn danger-combo' />
      ...
      <Modaljs open={districtShow} size={'lg'} closebtn={'Cancel'} Savebtn={'ADD'} close={() => setDistrictShow(false)} title={"Add District"}>
        ...
        <Form.Group className="mb-4">
          <Form.Label>Add District</Form.Label>
