...
const MergedPi = () => {
  ...
  const handleMergePi = async () => {
    const sData = {
      ...
      po_number: poId.id,
      complaint_for: rows[0]?.complaint_for,
      billing_from_state: rows[0]?.billing_from_state_id,
      financial_year: rows[0]?.financial_year,
    };
  ...
  const columns = useMemo(() => {
    ...
    return baseColumns;
  }, [poId?.id && regionalOfficceId?.id && salesAreaId.id, pageNo, pageSize]);
