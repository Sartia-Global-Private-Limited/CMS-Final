...
const ViewSalesOrder = () => {
  ...
  const fetchSingleData = async () => {
    const res = await getSingleSalesOrderById(id, { pageSize, pageNo, search });
    if (res.status) {
      setViewDetails(res?.data);
      setTotalData(res?.data?.sales_order_item?.pageDetails?.total);
    } else {
      setViewDetails({});
      setTotalData({});
    }
  ...
  const columns = useMemo(
    ...
    [t, ViewDetails?.sales_order_item?.data?.length, pageNo, pageSize]
  );
          ...
          {type === "sales-order" ? (
            <Row className="g-3 py-1">
              ...
              <Col md={12}>
                <CustomTable
                  id={"items"}
                  rows={ViewDetails?.sales_order_item?.data || []}
                  columns={columns}
                  pagination={{
                    pageNo,
                    pageSize,
