...
const MyCompanies = ({ checkPermission }) => {
  ...
  const fetchMyCompaniesData = async () => {
    const res = await getAdminMyCompanies({
      search,
      pageSize,
      pageNo,
      city: cityId?.label,
  ...
  const fetchCityData = async () => {
    const res = await getAllCitiesByCompany({ my_company: 1 });
    if (res.status) {
      setAllCity(
        res?.data?.map((city) => {
          ...
          <ActionButtons
            actions={{
              view: {
                ...
                action: () =>
                  navigate(
                    `/MyCompanies/ViewCompany/${info.row.original.company_id}`,
              ...
              edit: {
                ...
                action: () =>
                  navigate(
                    `/MyCompanies/AddMyCompany/${info.row.original.company_id}`
                  ),
              },
              delete: {
  ...
  return (
    <>
      ...
      <CustomTable
        ...
        excelAction={() => ""}
        pdfAction={() => ""}
        apiForExcelPdf={getAdminMyCompanies}
        customHeaderComponent={() => (
          <TableHeader
            ...
            button={{
              noDrop: true,
              to: `/MyCompanies/AddMyCompany/new`,
              title: "Add Company",
            }}
          />
        )}
        tableTitleComponent={
          <div>
            <UserPlus /> <strong>my company</strong>
          </div>
        }
        // hideCol={[
