...
import {
  getAllSurveyList,
  getSurveyList,
} from '../../redux/slices/survey/getAllSurveyListSlice';
import List from '../../component/List/List';
import {
  assignSurveyToEnergyCompany,
  updateSurveyStatus,
} from '../../redux/slices/survey/addUpdateSurveySlice';
import {getAdminAllSurvey} from '../../services/authApi';
...
const SurveyListScreen = ({navigation, route}) => {
  ...
  useEffect(() => {
    if (type == 'all') {
      dispatch(getAllSurveyList({pageSize: pageSize, pageNo: pageNo}));
    } else {
      dispatch(
        getSurveyList({
          pageSize: pageSize,
  ...
  const deleteWork = async workId => {
    try {
      const deleteResult = await dispatch(deleteSurvey(workId)).unwrap();
