...
const EnergyTeamListScreen = ({navigation, route}) => {
    ...
    switch (actionButton.typeOfButton) {
      case 'edit':
        navigation.navigate('AddUpdateEnergyTeamScreen', {
          edit_id: actionButton?.itemData.energy_company_id,
          user_id: actionButton?.itemData.user_id,
        });
        break;

      case 'delete':
        setECUserId(actionButton?.itemData?.user_id);
  ...
  const deleteEnergyCompanyUser = async id => {
    try {
      const deleteResult = await dispatch(
        deleteEnergyTeamUserById(id),
      ).unwrap();
      ...
      if (deleteResult?.status) {
        ...
        setDeleteModalVisible(false), setECUserId('');
        dispatch(
          getAllEnergyTeamList({
            id: companyId,
            search: searchText,
    ...
    <SafeAreaView style={{flex: 1, backgroundColor: Colors().screenBackground}}>
      ...
      {deleteModalVisible && (
        <AlertModal
          ...
          ConfirmBtnPress={() => deleteEnergyCompanyUser(id)}
        />
      )}
    </SafeAreaView>
  );
