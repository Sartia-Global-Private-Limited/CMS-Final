...
import {
  createSalesArea,
  updateSalesAreaById,
} from '../../../redux/slices/energyCompany/sales-area/getAllSalesAreaListSlice';

const AddUpdateSalesAreaScreen = ({navigation, route}) => {
  ...
  const formik = useFormik({
    ...
    initialValues: {
      ...
      regional_office_id: (item && item?.regional_office_id) || '',
      name: (item && item?.sales_area_name) || '',
  ...
  const handleSubmit = async values => {
    const reqBody = {
      ...
      sales_area_name: values?.name,
      zone_id: values?.zone_id,
      sales_area_status: values.status,
    ...
    try {
      setLoading(true);
      const res = item?.id
        ? await dispatch(updateSalesAreaById(reqBody)).unwrap()
        : await dispatch(createSalesArea(reqBody)).unwrap();
      ...
      if (res?.status) {
        ...
        setLoading(false);
        navigation.navigate('SalesAreaListScreen');
      } else {
        Toast.show({
          type: 'error',
          text1: res?.message,
  ...
  return (
    <SafeAreaView
      ...
      <CustomeHeader
        headerTitle={item?.id ? 'update Sales Area' : 'Add Sales Area'}
      />
...
export default AddUpdateSalesAreaScreen;
