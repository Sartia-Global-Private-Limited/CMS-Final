...
const OutletsListScreen = ({navigation, route}) => {
  ...
  const ListData = useSelector(state => state.allOutlets);
  ...
  const deleteZone = async zoneId => {
    try {
      const deleteResult = await dispatch(deleteOutletsById(zoneId)).unwrap();
  ...
  return (
    <SafeAreaView style={{flex: 1, backgroundColor: Colors().screenBackground}}>
      <CustomeHeader headerTitle={`All Outlets`} />
      ...
      <View style={{height: WINDOW_HEIGHT * 0.9, width: WINDOW_WIDTH}}>
        <List
          ...
          pageNo={pageNo}
          apiFunctions={() => {
            dispatch(allOutletsList({pageSize: 8, pageNo: pageNo}));
          }}
        />
      </View>
...
export default OutletsListScreen;
