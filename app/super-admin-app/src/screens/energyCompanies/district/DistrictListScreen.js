...
const DistrictListScreen = ({navigation, route}) => {
  ...
  const ListData = useSelector(state => state.allDistrict);
  ...
  const handleAction = actionButton => {
    switch (actionButton.typeOfButton) {
      case 'edit':
        navigation.navigate('AddUpdateDistrictScreen', {
  ...
  const deleteZone = async zoneId => {
    try {
      const deleteResult = await dispatch(deleteDistrictById(zoneId)).unwrap();
  ...
  return (
    <SafeAreaView style={{flex: 1, backgroundColor: Colors().screenBackground}}>
      <CustomeHeader headerTitle={`All Districts`} />
      ...
      <View style={{height: WINDOW_HEIGHT * 0.9, width: WINDOW_WIDTH}}>
        <List
          addAction={'AddUpdateDistrictScreen'}
          data={ListData}
          ...
          pageNo={pageNo}
          apiFunctions={() => {
            dispatch(allDistrictList({pageSize: 8, pageNo: pageNo}));
          }}
        />
      </View>
