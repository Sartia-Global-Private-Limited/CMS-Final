...
import {getItemNoFormatDetailById} from '../../../redux/slices/master-data-management/bill-format/getItemNoFormatDetailsSlice';
import {
  addItemNoFormat,
  updateItemNoFormat,
} from '../../../redux/slices/master-data-management/bill-format/addUpdateItemNoFormatSlice';

const AddUpdateItemNumberFormat = ({navigation, route}) => {
  ...
  const formik = useFormik({
    ...
    initialValues: {
      ...
      financial_year_format: edit.financial_year_format
        ...
        : '',
      start_item_number: edit?.start_item_number || '',
  ...
  const handleSubmit = async (values, resetForm) => {
    const reqBody = {
      ...
      start_item_number: values.start_item_number,
    ...
    try {
      setLoading(true);
      const res = edit_id
        ? await dispatch(updateItemNoFormat(reqBody)).unwrap()
        : await dispatch(addItemNoFormat(reqBody)).unwrap();
      ...
      if (res?.status) {
        ...
        setLoading(false);
        resetForm();
        navigation.navigate('ItemNumberFormatListScreen');
      } else {
        Toast.show({
          type: 'error',
          text1: res?.message,
  ...
  const fetchSingleDetails = async () => {
    const fetchResult = await dispatch(
      getItemNoFormatDetailById(edit_id),
    ).unwrap();
  ...
  return (
    <SafeAreaView
      ...
      <CustomeHeader
        headerTitle={
          edit_id ? 'update Item Number Format' : 'Add Item Number Format'
        }
      />
      ...
      <ScrollView>
        <View style={styles.inputContainer}>
          ...
          <View style={{rowGap: 8}}>
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
              <Text style={styles.title}>START Item NUMBER </Text>
              <Icon
                name="asterisk"
                type={IconType.FontAwesome}
                size={8}
            ...
            <NeumorphicTextInput
              placeHolderTxt={'TYPE...'}
              width={WINDOW_WIDTH * 0.9}
              value={formik?.values?.start_item_number}
              onChangeText={formik.handleChange('start_item_number')}
              style={styles.inputText}
              keyboardType={'numeric'}
            />
          </View>
          {formik?.touched?.start_item_number &&
            formik?.errors?.start_item_number && (
              <Text style={styles.errorMesage}>
                {formik?.errors?.start_item_number}
              </Text>
            )}
...
export default AddUpdateItemNumberFormat;
