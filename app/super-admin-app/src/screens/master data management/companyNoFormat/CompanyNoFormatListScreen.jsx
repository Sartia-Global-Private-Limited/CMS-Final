...
import {getAllCompanyNoFormatList} from '../../../redux/slices/master-data-management/bill-format/getCompanyNoFormatListSlice';
import {deleteCompanyNoFomartById} from '../../../redux/slices/master-data-management/bill-format/addUpdateCompanyNoFormatSlice';

const CompanyNoFormatListScreen = ({navigation, route}) => {
  /*declare hooks variable here */
  const dispatch = useDispatch();
  const isFocused = useIsFocused();
  const ListData = useSelector(state => state.getCompanyNoFormatList);
  ...
  const deleteBillFormat = async () => {
    try {
      const deleteResult = await dispatch(
        deleteCompanyNoFomartById(formatId),
      ).unwrap();
      ...
      if (deleteResult?.status === true) {
        ...
        dispatch(
          getAllCompanyNoFormatList({pageSize: pageSize, pageNo: pageNo}),
        );
      } else {
        Toast.show({
          type: 'error',
  ...
  const handleAction = actionButton => {
    switch (actionButton.typeOfButton) {
      case 'edit':
        navigation.navigate('AddUpdateCompanyNoFormats', {
    ...
    return (
      <CustomeCard
        ...
        data={[
          ...
          {
            key: 'start number',
            value: item?.start_company_number ?? '--',
            keyColor: Colors().pureBlack,
          },
          {
            key: 'sample format',
  ...
  const resetFunction = () => {
    dispatch(getAllCompanyNoFormatList({pageSize: pageSize, pageNo: pageNo}));
  };

  return (
    <SafeAreaView style={{flex: 1, backgroundColor: Colors().screenBackground}}>
      <CustomeHeader headerTitle={`Company Number Format`} />
      <View
        style={{
          flexDirection: 'row',
          alignItems: 'center',
...
export default CompanyNoFormatListScreen;
