...
const AddUpdateEmployeeNumberFormat = ({navigation, route}) => {
  ...
  const formik = useFormik({
    ...
    initialValues: {
      ...
      financial_year_format: edit.financial_year_format
        ...
        : '',
      start_employee_number: edit?.start_employee_number || '',
  ...
  const handleSubmit = async (values, resetForm) => {
    const reqBody = {
      ...
      start_employee_number: values.start_employee_number,
    ...
    try {
      setLoading(true);
      const res = edit_id
        ? await dispatch(updateEmployeeNoFormat(reqBody)).unwrap()
        : await dispatch(addEmployeeNoFormat(reqBody)).unwrap();
      ...
      if (res?.status) {
        ...
        setLoading(false);
        resetForm();
        navigation.navigate('EmployeeNumberFormatListScreen');
      } else {
        Toast.show({
          type: 'error',
          text1: res?.message,
  ...
  const fetchSingleDetails = async () => {
    const fetchResult = await dispatch(
      getEmployeeNoFormatDetailById(edit_id),
    ).unwrap();
  ...
  return (
    <SafeAreaView
      ...
      <CustomeHeader
        headerTitle={
          edit_id ? 'update Employee No Format' : 'Add Employee No Format'
        }
      />
      ...
      <ScrollView>
        <View style={styles.inputContainer}>
          ...
          <View style={{rowGap: 8}}>
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
              <Text style={styles.title}>START Employee NUMBER </Text>
              <Icon
                name="asterisk"
                type={IconType.FontAwesome}
                size={8}
            ...
            <NeumorphicTextInput
              placeHolderTxt={'TYPE...'}
              width={WINDOW_WIDTH * 0.9}
              value={formik?.values?.start_employee_number}
              onChangeText={formik.handleChange('start_employee_number')}
              style={styles.inputText}
              keyboardType={'numeric'}
            />
          </View>
          {formik?.touched?.start_employee_number &&
            formik?.errors?.start_employee_number && (
              <Text style={styles.errorMesage}>
                {formik?.errors?.start_employee_number}
              </Text>
            )}
...
export default AddUpdateEmployeeNumberFormat;
