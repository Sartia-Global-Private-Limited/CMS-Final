...
import MTIInvoiceFilter from './MTIInvoiceFilter';
import {
  getMTIFinalDiscardList,
  getMTIReadyToMergeList,
} from '../../../redux/slices/billing management/merge to invoice/getMTIListSlice';
import {
...
const MTIListScreen = ({navigation, route}) => {
  ...
  const ListData = useSelector(state => state.getMTIList);
  ...
  const discardOfMergedInvoice = async () => {
    const result = await dispatch(discardMergedInvoice(piId)).unwrap();
    if (result?.status) {
  ...
  /*for merging  invoice*/
  const mergerOfInvoice = async () => {
    const reqBody = {
      id: filterChekcBox.map(itm => itm?.id),
      po_number: poId,
      regional_office: roId,
      billing_from: billingFromId,
      billing_to: billingToId,
      companies_for: filterChekcBox[0]?.complaintFor,
    };

    const result = await dispatch(mergeInvoice(reqBody)).unwrap();
    if (result?.status) {
      Toast.show({
        type: 'success',
        text1: result?.message,
    ...
    <SafeAreaView style={{flex: 1, backgroundColor: Colors().screenBackground}}>
      ...
      {discardVisible && (
        <AlertModal
          ...
          ConfirmBtnPress={() => discardOfMergedInvoice()}
        />
      )}
    </SafeAreaView>
  );
