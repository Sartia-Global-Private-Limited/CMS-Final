...
const PerfromaInvoiceDetailScreen = ({navigation, route}) => {
  ...
  const ListData = useSelector(state => state.getPerformaInvoiceDetail);
  const dispatch = useDispatch();
  const all_Data = ListData?.data?.data || {};

  useEffect(() => {
    dispatch(getPIDetailById(pi_Id));
  }, [pi_Id]);
  ...
  const hideMenu = val => {
    ...
    switch (valueToSend) {
      case 'Itemlist':
        navigation.navigate('PIDetailItemListScreen', {
          pi_Id: pi_Id,
        });
        break;
  ...
  return (
    <SafeAreaView style={{flex: 1, backgroundColor: Colors().screenBackground}}>
      <CustomeHeader
        headerTitle={`${label.PERFORMA_INVOICE} ${label.DETAIL}`}
...
export default PerfromaInvoiceDetailScreen;
