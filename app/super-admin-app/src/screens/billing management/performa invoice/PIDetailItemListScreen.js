...
const PIDetailItemListScreen = ({navigation, route}) => {
  ...
  const {bill_no} = ListData?.data?.data || '';
  ...
  return (
    <SafeAreaView style={{flex: 1, backgroundColor: Colors().screenBackground}}>
      <CustomeHeader headerTitle={label.PI_ITEM_LIST} />
      {ListData?.isLoading ? (
        <Loader />
      ) : !ListData?.isLoading &&
        !ListData?.isError &&
...
export default PIDetailItemListScreen;
