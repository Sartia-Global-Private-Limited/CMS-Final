...
const SalesOrderDetailScreen = ({navigation, route}) => {
  ...
  const onRefresh = React.useCallback(() => {
    setRefreshing(true);
    setTimeout(() => {
      dispatch(getSalesOrderDetailById({soId: soId, pageNo: pageNo, pageSize}));
  ...
  const hideMenu = val => {
    ...
    switch (valueToSend) {
      case 'Itemlist':
        navigation.navigate('SoDetaiItemListScreen', {
          soId: soId,
        });
        break;
  ...
  return (
    <SafeAreaView style={{flex: 1, backgroundColor: Colors().screenBackground}}>
      <CustomeHeader
        headerTitle={`${label.SALE} ${label.ORDER} ${label.DETAIL}`}
        ...
        <>
          <ScrollView
            ...
            <View style={{}}>
              ...
              <CustomeCard
                headerName={'sO DETAILS'}
                data={[
                  {
                    key: 'so date',
                    value: all_Data?.so_date,
                  },
                  {
                    key: 'so Regional office',
                  ...
                  },
                  {
                    ...
                    value: `₹ ${all_Data?.remaining_so_amount}`,
