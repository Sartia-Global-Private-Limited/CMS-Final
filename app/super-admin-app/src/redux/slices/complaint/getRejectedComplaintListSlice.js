  ...
  async ({
    ...
    try {
      const {data} = isDropdown
        ? await customApi.get(
            `/api/super-admin/complaints/get-rejected-complaints?isDropdown=${isDropdown}`,
          )
        : await customApi.get(
            `/api/super-admin/complaints/get-rejected-complaints?search=${search}&pageSize=${pageSize}&pageNo=${pageNo}&sales_area_id=${sales_area_id}&outlet_id=${outlet_id}&regional_office_id=${regional_office_id}&order_by_id=${order_by_id}&company_id=${company_id}&complaint_for=${complaint_for}`,
          );
