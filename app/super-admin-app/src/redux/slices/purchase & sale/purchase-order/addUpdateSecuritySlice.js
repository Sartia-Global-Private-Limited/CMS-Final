  ...
  async status => {
    try {
      const {data} = await customApi.get(
        `/api/super-admin/get-ro-for-po?status=${status || ''}&&po_id=`,
      );
...
export const getAllPoForSecurityFilter = createAsyncThunk(
  ...
  async ({status, ro}) => {
    try {
      const {data} = await customApi.get(
        `/api/super-admin/get-po-number-for-po?status=${status || ''}&&ro=${
          ro || ''
        }`,
      );
...
export const getAllSecurityForSecurityFilter = createAsyncThunk(
  ...
  async ({status, po}) => {
    try {
      const {data} = await customApi.get(
        `/api/super-admin/get-security-unique-id?status=${status || ''}&&po=${
          po || ''
        }`,
      );
  ...
  async reqBody => {
    try {
      const {data} = await customApi.post(
        `/api/super-admin/approve-purchase-order`,
        reqBody,
      );
...
export const updateSecurityPurchaseOrder = createAsyncThunk(
  ...
  async reqBody => {
    try {
      const {data} = await customApi.post(
        `/api/super-admin/approve-update-purchase-order`,
        reqBody,
      );
