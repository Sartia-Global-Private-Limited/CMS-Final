...
// Assuming you've installed the necessary AsyncStorage package
...
export const tokenErrorToast = api => next => action => {
  if (isRejectedWithValue(action)) {
    console.error('Authentication Failed', isRejectedWithValue(action));
    Toast.show({
      type: 'error',
    ...
    AsyncStorage.removeItem('cms-client-token');
  }
  return next(action);
};
