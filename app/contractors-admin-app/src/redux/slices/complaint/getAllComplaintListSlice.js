  ...
  async ({
    ...
    try {
      const {data} = isDropdown
        ? await customApi.get(
            `/api/contractor/complaints/get-all-complaints?isDropdown=${isDropdown}`,
          )
        : await customApi.get(
            `/api/contractor/complaints/get-all-complaints?search=${search}&pageSize=${pageSize}&pageNo=${pageNo}&sales_area_id=${sales_area_id}&outlet_id=${outlet_id}&regional_office_id=${regional_office_id}&order_by_id=${order_by_id}&area_manager_id=${area_manager_id}&supervisor_id=${supervisor_id}&end_user_id=${end_user_id}&company_id=${company_id}&complaint_for=${complaint_for}`,
          );
