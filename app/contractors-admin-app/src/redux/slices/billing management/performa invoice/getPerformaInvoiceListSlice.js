  ...
  async ({
    ...
    try {
      const { data } = await axios.get(
        `${apiBaseUrl}/api/contractor/billing/performa-invoice/get-measurements-in-pi-status?status=${
          status || ''
          ...
          search || ''
        }&&pageSize=${pageSize || ''}&&pageNo=${pageNo || ''}&&sale_area_id=${salesAreaId}&&financial_year=${financialYear}&& outlet_id=${oId}&&complaint_type=${complaintTypeId}`,
        {
          headers: {
...
export const getPiFinalDiscardList = createAsyncThunk(
  ...
  async ({
    ...
    try {
      const { data } = await axios.get(
        `${apiBaseUrl}/api/contractor/billing/performa-invoice/get-all-proforma-invoices?status=${
          status || ''
