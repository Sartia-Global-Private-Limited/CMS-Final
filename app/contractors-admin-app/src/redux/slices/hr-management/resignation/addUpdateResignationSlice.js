  ...
  async ({ resignationId, statusId }) => {
    try {
      const { data } = await customApi.post(
        `/api/contractor/payroll/resignation/update-resignations-request-by-admin/${resignationId}/${statusId}`,
      );
...
export const createResignation = createAsyncThunk(
  ...
  async reqBody => {
    try {
      const { data } = await customApi.post(
        `/api/contractor/payroll/resignation/sub-user/register-employee-resignation`,
        reqBody,
      );
