...
import {getAllEmployeeNoFormatList} from '../../../redux/slices/master-data-management/bill-format/getEmployeeNoFormatListSlice';
import {deleteEmployeeNoFomartById} from '../../../redux/slices/master-data-management/bill-format/addUpdateEmployeeNoFormatSlice';

const EmployeeNumberFormatListScreen = ({navigation, route}) => {
  /*declare hooks variable here */
  const dispatch = useDispatch();
  const isFocused = useIsFocused();
  const ListData = useSelector(state => state.getEmployeeNoFormatList);
  ...
  const deleteBillFormat = async () => {
    try {
      ...
      if (deleteResult?.status === true) {
        ...
        dispatch(
          getAllEmployeeNoFormatList({pageSize: pageSize, pageNo: pageNo}),
        );
      } else {
        Toast.show({
          type: 'error',
  ...
  const handleAction = actionButton => {
    switch (actionButton.typeOfButton) {
      case 'edit':
        navigation.navigate('AddUpdateEmployeeNumberFormat', {
    ...
    return (
      <CustomeCard
        ...
        data={[
          ...
          {
            key: 'start number',
            value: item?.start_employee_number ?? '--',
            keyColor: Colors().pureBlack,
          },
          {
            key: 'sample format',
  ...
  const resetFunction = () => {
    dispatch(getAllEmployeeNoFormatList({pageSize: pageSize, pageNo: pageNo}));
  };

  return (
    <SafeAreaView style={{flex: 1, backgroundColor: Colors().screenBackground}}>
      <CustomeHeader headerTitle={`Employee Number Format`} />
      <View
        style={{
          flexDirection: 'row',
          alignItems: 'center',
...
export default EmployeeNumberFormatListScreen;
