...
import {getEmployeeNoFormatDetailById} from '../../../redux/slices/master-data-management/bill-format/getEmployeeNoFormatDetailSlice';
import {
  addEmployeeNoFormat,
  updateEmployeeNoFormat,
} from '../../../redux/slices/master-data-management/bill-format/addUpdateEmployeeNoFormatSlice';
import {getCompanyNoFormatDetailById} from '../../../redux/slices/master-data-management/bill-format/getCompanyNoFormatDetailSlice';
import {
  addCompanyNoFormat,
  updateCompanyNoFormat,
} from '../../../redux/slices/master-data-management/bill-format/addUpdateCompanyNoFormatSlice';

const AddUpdateCompanyNoFormat = ({navigation, route}) => {
  ...
  const handleSubmit = async (values, resetForm) => {
    ...
    try {
      setLoading(true);
      const res = edit_id
        ? await dispatch(updateCompanyNoFormat(reqBody)).unwrap()
        : await dispatch(addCompanyNoFormat(reqBody)).unwrap();
      ...
      if (res?.status) {
        ...
        setLoading(false);
        resetForm();
        navigation.navigate('CompanyNoFormatListScreen');
      } else {
        Toast.show({
          type: 'error',
          text1: res?.message,
  ...
  const fetchSingleDetails = async () => {
    const fetchResult = await dispatch(
      getCompanyNoFormatDetailById(edit_id),
    ).unwrap();
  ...
  return (
    <SafeAreaView
      ...
      <CustomeHeader
        headerTitle={
          edit_id ? 'update Company No. Format' : 'Add Company No. Format'
        }
      />
      ...
      <ScrollView>
        <View style={styles.inputContainer}>
          ...
          <View style={{rowGap: 8}}>
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
              <Text style={styles.title}>START Company NUMBER </Text>
              <Icon
                name="asterisk"
                type={IconType.FontAwesome}
                size={8}
            ...
            <NeumorphicTextInput
              placeHolderTxt={'TYPE...'}
              width={WINDOW_WIDTH * 0.9}
              value={formik?.values?.start_company_number}
              onChangeText={formik.handleChange('start_company_number')}
              style={styles.inputText}
              keyboardType={'numeric'}
            />
          </View>
          {formik?.touched?.start_company_number &&
            formik?.errors?.start_company_number && (
              <Text style={styles.errorMesage}>
                {formik?.errors?.start_company_number}
              </Text>
            )}
...
export default AddUpdateCompanyNoFormat;
