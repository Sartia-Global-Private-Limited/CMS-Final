...
import { getAllBillFormatList } from '../../../redux/slices/master-data-management/bill-format/getBillFormatListSlice';
...
import { deleteBillFomartById } from '../../../redux/slices/master-data-management/bill-format/addUpdateBillFormatSlice';
import List from '../../../component/List/List';
import CustomeCard from '../../../component/CustomeCard';

const BillFormatListScreen = ({ navigation, route }) => {
  /* declare props constant variale*/

  /*declare hooks variable here */
  const dispatch = useDispatch();
  const isFocused = useIsFocused();
  const ListData = useSelector(state => state.getBillFormatList);
  ...
  const deleteBillFormat = async () => {
    try {
      ...
      if (deleteResult?.status === true) {
        ...
        dispatch(getAllBillFormatList({ pageSize: pageSize, pageNo: pageNo }));
      } else {
        Toast.show({
          type: 'error',
          text1: deleteResult?.message,
  ...
  const handleAction = actionButton => {
    switch (actionButton.typeOfButton) {
      case 'edit':
        navigation.navigate('AddUpdateBillFormatScreen', {
    ...
    return (
      <CustomeCard
        ...
        data={[
          ...
          {
            key: 'start bill number',
            value: item?.start_bill_number ?? '--',
            keyColor: Colors().pureBlack,
          },
          {
            key: 'sample format',
  ...
  return (
    <SafeAreaView
      ...
      <CustomeHeader headerTitle={`Bill Number Format`} />
      <View
        style={{
          flexDirection: 'row',
          alignItems: 'center',
      ...
      <View style={{ height: WINDOW_HEIGHT * 0.9, width: WINDOW_WIDTH }}>
        <List
          addAction={'AddUpdateBillFormatScreen'}
          data={ListData}
...
export default BillFormatListScreen;
