...
import {addOutletsSchema} from '../../utils/FormSchema';
...
const AddUpdateOutletScreen = ({route}) => {
  ...
  const handleSubmit = async (values, resetForm) => {
    ...
    try {
      setLoading(true);
      const res = edit_id
        ? await dispatch(updateOutlet(formData)).unwrap()
        : await dispatch(addOutlet(formData)).unwrap();
  ...
  const fetchSingleData = async () => {
    try {
      const result = await dispatch(getOutletDetailById(edit_id)).unwrap();
