...
import { getAllBrandList } from '../../../redux/slices/item master/brand/getBrandListSlice';
import { deleteBrandById } from '../../../redux/slices/item master/brand/addUpdateBrandSlice';
...
const BrandListScreen = ({ navigation, route }) => {
  ...
  const listData = useSelector(state => state.getBrandList);
  ...
  const deleteBrand = async brandId => {
    try {
      const deleteResult = await dispatch(deleteBrandById(brandId)).unwrap();
      ...
      if (deleteResult?.status === true) {
        ...
        dispatch(getAllBrandList({ pageSize: pageSize, pageNo: pageNo }));
      } else {
        Toast.show({
          type: 'error',
          text1: deleteResult?.message,
  ...
  const handleAction = actionButton => {
    switch (actionButton.typeOfButton) {
      case 'edit':
        navigation.navigate('AddUpdateBrandScreen', {
  ...
  return (
    <SafeAreaView
      ...
      <CustomeHeader headerTitle={`${label.ALL_BRAND}`} />
      ...
      <View style={{ height: WINDOW_HEIGHT * 0.9, width: WINDOW_WIDTH }}>
        <List
          ...
          addAction={'AddUpdateBrandScreen'}
        />
      </View>
