...
import {getSPRequestDetailById} from '../../../redux/slices/stock-punch-management/stock-punch-request/getSPRequestDetailSlice';
import ScreensLabel from '../../../constants/ScreensLabel';

const SPRequestDetailScreen = ({navigation, route}) => {
  /* declare props constant variale*/
  const edit_id = route?.params?.edit_id;
  const label = ScreensLabel();
  /*declare hooks variable here */
  const dispatch = useDispatch();
  const ListData = useSelector(state => state.getSPRequestDetail);
  ...
  return (
    <SafeAreaView style={{flex: 1, backgroundColor: Colors().screenBackground}}>
      <CustomeHeader
        headerTitle={`${label.SP_REQUEST}  ${label.DETAIL}`}
...
export default SPRequestDetailScreen;
