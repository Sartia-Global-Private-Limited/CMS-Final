...
const StockRequestTransferDataScreen = ({navigation, route}) => {
  ...
  /*funtction for getting total of price*/
  const getTotalPrice = (data, key1, key2) => {
    let total = 0;
    data.forEach(element => {
      total += parseFloat(element[key1] * element[key2]) || 0;
    });
    return total;
  };
  ...
  /*function  for getting Final of approve qty and amount of approved item*/
  const getFinal = (data, key) => {
    let totalOfOldItem = 0;
    let totalOfNewItem = 0;

    if (data?.request_stock) {
      totalOfOldItem = getTotal(data?.request_stock, key);
    }
    if (data?.new_request_stock) {
      totalOfNewItem = getTotal(data?.new_request_stock, key);
    }
    return totalOfOldItem + totalOfNewItem;
  };
  ...
  return (
    <SafeAreaView style={{flex: 1, backgroundColor: Colors().screenBackground}}>
      <CustomeHeader headerTitle={label.TRANSFERED_STOCK_DATA} />
          ...
          {data?.transfer_stocks ? (
            <ScrollView
              ...
              <View style={styles.mainView}>
                      ...
                      <View style={{flex: 1, justifyContent: 'center'}}>
                        ...
                        {data?.request_stock_images && (
                          <View style={{flexDirection: 'row'}}>
                            ...
                            <View
                              style={[styles.userNameView, {columnGap: 10}]}>
                ...
                {/*transfer items card */}
                {data?.transfer_stocks?.request_stock && (
                  <NeumorphCard>
                    <View style={styles.cardContainer}>
                      <Text
                      ...
                      <ScrollView
                        ...
                        <DataTable>
                          ...
                          <ScrollView>
                            {data?.transfer_stocks?.request_stock.map(
                              (itm, index) => (
                                <>
                ...
                {/*New transfer items card  */}
                {data?.transfer_stocks?.new_request_stock &&
                  !isObjectEmpty(data?.transfer_stocks?.new_request_stock) && (
                    <NeumorphCard>
                      <View style={styles.cardContainer}>
                        <Text
                          style={[styles.headingTxt, {color: Colors().purple}]}>
                        ...
                        <ScrollView
                          ...
                          <DataTable>
                            ...
                            <ScrollView>
                              {data?.transfer_stocks?.new_request_stock.map(
                                (itm, index) => (
                                  <>
                                    <DataTable.Row key={index} style={{}}>
                                      <DataTable.Cell
...
export default StockRequestTransferDataScreen;
