...
import { getAreaManagerComplaints } from '../../redux/slices/commonApi';
import { useDispatch } from 'react-redux';
import Colors from '../../constants/Colors';
import ChartCard from '../../component/ChartCard';

const AreaManagerComplaintDetail = ({ selectedFY }) => {
  ...
  const fetchAllData = async () => {
    try {
      const res = await dispatch(getAreaManagerComplaints(selectedFY)).unwrap();
      if (res?.status) {
        setData(res?.data);
      } else {
        setData([]);
  ...
  return (
    <SafeAreaView>
      <ChartCard
        headerName={'Area Manager Complaint details'}
        data={[
          {
            component: (
...
export default AreaManagerComplaintDetail;
