...
import {
  addCategory,
  updateCategory,
} from '../../../redux/slices/category&product/category/addUpdateCategorySlice';
import { getCategoryDetailById } from '../../../redux/slices/category&product/category/getCategoryDetailSlice';
import ScreensLabel from '../../../constants/ScreensLabel';
const AddUpdateCategory = ({ navigation, route }) => {
  ...
  const handleSubmit = async (values, resetForm) => {
    ...
    try {
      setLoading(true);
      const res = edit_id
        ? await dispatch(updateCategory(reqbody)).unwrap()
        : await dispatch(addCategory(reqbody)).unwrap();
  ...
  const fetchSingleData = async () => {
    const res = await dispatch(getCategoryDetailById(edit_id)).unwrap();
    if (res.status) {
      setEdit(res.data);
    } else {
      setEdit({});
  ...
  return (
    <SafeAreaView
      ...
      <CustomeHeader
        headerTitle={
          edit_id
            ? `${label.CATEGORY} ${label.UPDATE}`
            : `${label.CATEGORY} ${label.ADD}`
        }
      />
      ...
      <ScrollView>
        <View style={styles.inpuntContainer}>
          <NeumorphicTextInput
            ...
            errorMessage={formik.errors.category_name}
          />
