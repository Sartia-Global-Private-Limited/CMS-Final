...
import SiteStockInspectionFilter from './SiteStockInspectionFilter';
import {
  getApprovedSiteStockInspection,
  getPartialSiteStockInspection,
  getPendingSiteStockInspection,
} from '../../../redux/slices/site-inspection/site-stock-inspection/getSiteStockInspectionListSlice';
import List from '../../../component/List/List';

const SiteStockInspectionListingScreen = ({ navigation, route }) => {
  ...
  const ListData = useSelector(state => state.getSiteStockInspectionList);
  ...
  const handleAction = actionButton => {
    switch (actionButton.typeOfButton) {
      case 'approve':
        navigation.navigate('SiteStockInspectionDetailScreen', {
  ...
  const handlePageClick = () => {
    if (type == 'pending') {
      dispatch(
        getPendingSiteStockInspection({ pageSize: pageSize, pageNo: pageNo }),
      );
    }
    if (type == 'partial') {
      dispatch(
        getPartialSiteStockInspection({ pageSize: pageSize, pageNo: pageNo }),
      );
    }
    if (type == 'approved') {
      dispatch(
...
export default SiteStockInspectionListingScreen;
