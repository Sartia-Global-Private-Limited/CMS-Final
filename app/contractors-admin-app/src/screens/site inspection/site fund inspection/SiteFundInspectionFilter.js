...
const SiteFundInspectionFilter = ({
  type,
  roId,
  setRoId,
  saId,
  ...
  const fetchAllRegionalOffice = async () => {
    try {
      const result = await dispatch(
        getAllRoSiteFundInspection({ statusCode: getStatus(type) }),
      ).unwrap();
      if (result?.status) {
  ...
  const fetchAllSalesArea = async () => {
    try {
      const result = await dispatch(
        getAllSaSiteFundInspection({ statusCode: getStatus(type) }),
      ).unwrap();
      if (result?.status) {
        const rData = result?.data.map(itm => ({
  ...
  const fetchAllOutlet = async () => {
    try {
      const result = await dispatch(
        getAllOutletSiteFundInspection({ statusCode: getStatus(type) }),
      ).unwrap();
      if (result?.status) {
        const rData = result?.data.map(itm => ({
...
export default SiteFundInspectionFilter;
