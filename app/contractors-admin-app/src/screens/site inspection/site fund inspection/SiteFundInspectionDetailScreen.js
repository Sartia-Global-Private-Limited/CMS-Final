...
import SiteFundInspectionItemList from './SiteFundInspectionItemList';
import NeumorphicButton from '../../../component/NeumorphicButton';
import { useFormik } from 'formik';
import InspectionFeedbackForm from '../../../component/InspectionFeedbackForm';
import { approveSiteFundInspection } from '../../../redux/slices/site-inspection/site-fund-inspection/approveSiteFundInspectionSlice';
import {
  getApprovedSiteFundInspectionDetail,
  getPartialSiteFundInspectionDetail,
  getPendingSiteFundInspectionDetail,
} from '../../../redux/slices/site-inspection/site-fund-inspection/getSiteFundInspectionDetailSlice';

const SiteFundInspectionDetailScreen = ({ navigation, route }) => {
  ...
  const ListData = useSelector(state => state.getSiteFundInspectionDetail);
  ...
  const handleSubmit = async (values, resetForm) => {
    ...
    try {
      setLoading(true);
      const res = await dispatch(approveSiteFundInspection(sData)).unwrap();
      ...
      if (res?.status) {
        ...
        resetForm();
        navigation.navigate('SiteFundInspectionTopTab');
      } else {
        Toast.show({
          type: 'error',
          text1: res?.message,
...
export default SiteFundInspectionDetailScreen;
