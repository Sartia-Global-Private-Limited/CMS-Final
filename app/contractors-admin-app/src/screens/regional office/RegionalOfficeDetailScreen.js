...
import { getROPaidDetailById } from '../../redux/slices/regional office/getROPaymentPaidDetailSlice';

const RegionalOfficeDetailScreen = ({ navigation, route }) => {
  const id = route?.params?.id;
  const label = ScreensLabel();
  const [refreshing, setRefreshing] = useState(false);
  const ListData = useSelector(state => state.getROPaymentPaidDetail);
  const dispatch = useDispatch();
  const all_Data = ListData?.data?.data?.[0] || {};

  useEffect(() => {
    dispatch(getROPaidDetailById(id));
  }, [id]);
    ...
    <View style={{}}>
      ...
      <CustomeCard
        ...
        data={[
          ...
          {
            key: 'po no',
            value: all_Data?.po_details?.po_number ?? ' - - -',
          },
        ]}
        status={[
          {
  ...
  return (
    <SafeAreaView
      ...
      <CustomeHeader headerTitle={`${label.REGIONAL_OFFICE} ${label.DETAIL}`} />
...
export default RegionalOfficeDetailScreen;
