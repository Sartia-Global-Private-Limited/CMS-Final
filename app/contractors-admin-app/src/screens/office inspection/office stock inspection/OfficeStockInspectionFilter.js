...
const OfficeStockInspectionFilter = ({
  type,
  roId,
  setRoId,
  saId,
  ...
  const fetchAllRegionalOffice = async () => {
    try {
      const result = await dispatch(
        getAllRoOfficeStockInspection({ statusCode: getStatus(type) }),
      ).unwrap();
      if (result?.status) {
  ...
  const fetchAllSalesArea = async () => {
    try {
      const result = await dispatch(
        getAllSaOfficeStockInspection({ statusCode: getStatus(type) }),
      ).unwrap();
      if (result?.status) {
        const rData = result?.data.map(itm => ({
  ...
  const fetchAllOutlet = async () => {
    try {
      const result = await dispatch(
        getAllOutletOfficeStockInspection({ statusCode: getStatus(type) }),
      ).unwrap();
      if (result?.status) {
        const rData = result?.data.map(itm => ({
...
export default OfficeStockInspectionFilter;
