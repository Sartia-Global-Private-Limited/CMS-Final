...
} from '../../../redux/slices/office-inspection/office-stock-inspection/getOfficeStockInspectionDetailSlice';
import OfficeStockInspectionItemList from './OfficeStockInspectionItemList';
import NeumorphicButton from '../../../component/NeumorphicButton';
import { useFormik } from 'formik';
import InspectionFeedbackForm from '../../../component/InspectionFeedbackForm';
import { approveOfficeStockInspection } from '../../../redux/slices/office-inspection/office-stock-inspection/approveOfficeStockInspectionSlice';

const OfficeStockInspectionDetailScreen = ({ navigation, route }) => {
  ...
  const ListData = useSelector(state => state.getOfficeStockInspectionDetail);
  ...
  const handleSubmit = async (values, resetForm) => {
    ...
    try {
      setLoading(true);
      const res = await dispatch(approveOfficeStockInspection(sData)).unwrap();
      ...
      if (res?.status) {
        ...
        resetForm();
        navigation.navigate('OfficeStockInspectionTopTab');
      } else {
        Toast.show({
          type: 'error',
          text1: res?.message,
...
export default OfficeStockInspectionDetailScreen;
