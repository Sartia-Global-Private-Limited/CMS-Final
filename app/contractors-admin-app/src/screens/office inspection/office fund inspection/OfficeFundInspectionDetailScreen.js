...
} from '../../../redux/slices/office-inspection/office-fund-inspection/getOfficeFundInspectionDetailSlice';
import OfficeFundInspectionItemList from './OfficeFundInspectionItemList';
import { approveOfficeFundInspection } from '../../../redux/slices/office-inspection/office-fund-inspection/approveOfficeFundInspectionSlice';

const OfficeFundInspectionDetailScreen = ({ navigation, route }) => {
  ...
  const ListData = useSelector(state => state.getOfficeFundInspectionDetail);
  ...
  const handleSubmit = async (values, resetForm) => {
    ...
    try {
      setLoading(true);
      const res = await dispatch(approveOfficeFundInspection(sData)).unwrap();
      ...
      if (res?.status) {
        ...
        resetForm();
        navigation.navigate('OfficeFundInspectionTopTab');
      } else {
        Toast.show({
          type: 'error',
          text1: res?.message,
  ...
  const handleAction = actionButton => {
    ...
    const { item } = actionButton?.itemData;
    ...
    switch (actionButton.typeOfButton) {
      case 'list':
        ...
        break;
      case 'feedback':
        if (!formik?.values?.feedback?.[index]?.complaint_unique_id) {
          formik.setFieldValue(`feedback.${index}`, {
            ...
            complaints: '',
            suggestions: '',
            complaint_unique_id: item?.itemDetails?.[0]?.complaint_unique_id,
  ...
  const renderItem = ({ item, index }) => {
    return (
      <View>
        <CustomeCard
          allData={{ item, index, showTable, showFeedback }}
  ...
  return (
    <SafeAreaView
      ...
      <CustomeHeader
        headerTitle={
          purpose == 'approve'
            ? `${label.APPROVE} ${label.OFFICE_FUND}`
            : `${label.FUND_INSPECTION} ${label.DETAIL}`
        }
      />
      {ListData?.isLoading ? (
        <Loader />
...
export default OfficeFundInspectionDetailScreen;
