...
import OfficeFundInspectionFilter from './OfficeFundInspectionFilter';
import {
  getApprovedOfficeFundInspection,
  getPartialOfficeFundInspection,
  getPendingOfficeFundInspection,
} from '../../../redux/slices/office-inspection/office-fund-inspection/getOfficeFundInspectionListSlice';
import List from '../../../component/List/List';

const OfficeFundInspectionListingScreen = ({ navigation, route }) => {
  ...
  const ListData = useSelector(state => state.getOfficeFundInspectionList);
  ...
  const handleAction = actionButton => {
    switch (actionButton.typeOfButton) {
      case 'approve':
        navigation.navigate('OfficeFundInspectionDetailScreen', {
  ...
  const handlePageClick = () => {
    if (type == 'pending') {
      dispatch(
        getPendingOfficeFundInspection({ pageSize: pageSize, pageNo: pageNo }),
      );
    }
    if (type == 'partial') {
      dispatch(
        getPartialOfficeFundInspection({ pageSize: pageSize, pageNo: pageNo }),
      );
    }
    if (type == 'approved') {
      dispatch(
...
export default OfficeFundInspectionListingScreen;
